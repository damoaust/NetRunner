...
var _renderer: CP2020NeonGridRenderer = null


func _ready() -> void:
	# net_time_seconds is NOT reset here — per ARCHITECTURE, it is preserved
	# across in-datafort LDL travel and the datafort→City Grid return (mid-run).
	# The run-start reset lives in the world map scene; the run-end reset
	# lives in _return_to_world_map / _jack_out_to_hub.
	_build_grid()
	runner_pos = _resolve_entry()
	# Rendering is delegated to a shared CP2020NeonGridRenderer child so the
	# runtime and the @tool designer draw from one code path. The child's
	# _process drives the pulse animation and continuous redraws.
	_renderer = CP2020NeonGridRenderer.new()
	add_child(_renderer)
	_sync_renderer()
	if turn_manager:
...
# Push the runtime grid state into the shared renderer child so a single code
# path draws both this runtime node and the @tool designer.
func _sync_renderer() -> void:
	if _renderer == null:
		return
	_renderer.grid_cols = grid_cols
	_renderer.grid_rows = grid_rows
	_renderer.grid_offset = SCREEN_OFFSET
	_renderer.dataforts = dataforts
	_renderer.city_name = city_name
	_renderer.header_label = "CITY GRID"
	_renderer.ldl_entry = _resolve_entry()
	_renderer.selected_datafort_pos = null
	_renderer.runner_pos = runner_pos
