...
- **States**: `IDLE` -> `PURSUE`. Activates upon turn execution, taking up to `max_ap` (3) steps per turn toward the Netrunner's position. On reaching the runner, branches on `effect_type`: `DAMAGE_RUNNER` emits `attacked_netrunner(strength)` (anti-personnel health attack); `DEREZ_ICE` emits `attacked_program(strength)` (anti-program attack that damages an installed program instead — see §5.3 program-HP model).
