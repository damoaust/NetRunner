...
- **Properties**:
  ...
  - **Per-tile ICE overrides** (BLACK_ICE tiles; authored in the datafort designer's ICE editor; zero/empty = use the hub security-tier template):
    - `ice_program: NetProgram` — optional assigned `program.tres`. When set it supplies the ICE's `program_name` / `strength` / `effect_type` (the `effect_type` **drives in-game behavior**: `DAMAGE_RUNNER` = anti-personnel health attack; `DEREZ_ICE` = anti-program attack on the runner's installed programs). The program is `duplicate()`d at spawn so the cached `.tres` is never mutated.
    - `ice_program_name: String`, `ice_strength: int`, `ice_max_ap: int`, `ice_max_integrity: int`, `ice_traces: bool` — scalar overrides (`max_ap`/`max_integrity`/`traces` apply even when a program is assigned, since `NetProgram` has no equivalents; `ice_program_name`/`ice_strength` are legacy, used only when no program is assigned)
...
- **Properties**: `program_name`, `type`, `effect_type`, `memory_cost` (MU), `strength`, `price`, `icon`, `description` (one-line summary shown in the workbench detail card), `damage_dice` (optional: `0` = use flat `strength` as damage on each Black ICE hit, the default for all existing programs; `>0` = roll `1D{damage_dice}` per hit instead, e.g. **Sword** sets `6` to roll 1D6 per hit). Backward-compatible: every existing `.tres` keeps flat-strength damage without re-saving.
...
- **Worm Mechanics** (`execute_worm(program, target_coord)` / `_tick_worm_programs()`):
  - `execute_worm` is called from `_on_action_triggered` when a `WORM`-effect program is used on a `DATAWALL` or locked `CODE_GATE` tile. It validates the target tile type, sets `tile.worm_turns_remaining = 2`, logs the action, and triggers a redraw. The tile is **not** opened immediately — the worm works over 2 turns. **No trace increase, no ICE activation** (stealth opener).
  - `_tick_worm_programs()` is called at the start of each netrunner turn (by `_on_turn_ended`). It iterates all grid tiles, decrements `worm_turns_remaining`, and when a tile reaches 0 it opens it: `DATAWALL` → converted to `EMPTY`, `CODE_GATE` → `is_unlocked = true`. It then logs the opening, recalculates fog of war, and redraws the board.
...
- **Armor & Shield defense**: `active_armor` (persistent point-for-point absorber) and `raised_shield` (one-shot opposed-roll blocker). In `apply_damage`, Armor absorbs FIRST (`min(attack_strength, active_armor.strength)`), then any remainder goes to the Shield opposed roll (ties → attacker), then HP. Armor is NOT consumed on a hit. Shield is consumed after one roll.
- **Anti-personnel damage resolution**: `apply_damage(..., is_anti_personnel = true)` (called from `_on_ice_attacked`) applies, after HP loss: (1) INT-stat loss of 1 per hit (`int_changed` signal), and (2) a Mortal/Stun save — `1D10 + body` vs a target scaled to cumulative damage (0-3 → 8, 4-7 → 12, 8+ → 15); failure emits `stunned(1)`. Flatline is still governed solely by `current_health <= 0`.
...
- **Program-HP model** (`program_integrity: Dictionary`): a program's `strength` is also its max health (integrity). Installed programs are seeded to `prog.strength` on `install_program` and erased on `uninstall_program`. `damage_program(amount, attacker)` is invoked by anti-program (`DEREZ_ICE`) ICE attacks: it picks a random installed program with integrity > 0, reduces it, and uninstalls (destroys) it at 0 integrity (emitting `deck_updated` so the HUD refreshes MU). The raised shield does **not** block anti-program attacks (shields protect the runner's persona/health, not programs). If the runner has no installed programs, a `DEREZ_ICE` attack logs "no programs to target" and does nothing (no fallback to health damage).
...
- **States**: `IDLE` -> `PURSUE`. Activates upon turn execution, taking up to `max_ap` (3) steps per turn toward the Netrunner's position. On reaching the runner, branches on `effect_type`: `DAMAGE_RUNNER` emits `attacked_netrunner(damage)` (anti-personnel health attack); `DEREZ_ICE` emits `attacked_program(damage)` (anti-program attack that damages an installed program instead — see §5.3 program-HP model). The emitted `damage` is computed by `_roll_damage()`: `strength` when `damage_dice <= 0` (flat-strength, the legacy default), otherwise `randi_range(1, damage_dice)` — a per-hit dice roll sourced from the assigned program (e.g. **Sword** rolls 1D6 per hit). This means "BlackICE attacks work by the assigned program type, not a general rule" — the program, not a global rule, defines how damage is dealt.
...
- **Menu branches** (first match wins):
  ...
  - **Runner's own tile** (visible): offer `SHIELD` defense programs (ids `1000+i`), `ARMOR` defense programs (ids `7000+i`), and `DETECTION` programs (ids `1000+i`) labelled `"Watchdog (Deploy STR X, X MU)"`. Armor is a persistent passive absorber; raising it does not consume a turn action. A `DETECTION` program consumes 1 action to deploy a Watchdog beacon at the runner's current position (see §5.1 Watchdog Beacons). A program already deployed this run is filtered out of the offered list (one file, one instance — load a second copy at the workbench to run two beacons).
...
- `_on_menu_action_selected` checks ids in this order to avoid collision: LDL travel (`3000`/`3001`) → NPC talk (`4000`) → NPC attack (`2000+i`) → CPU crash (`5000+i`) → **memory files (`6000+i` per file / `6999` Copy All)** → **Armor-raise (`7000+i`)** → `CONTROL_NODE` loot (`loot_tile`) → program use (`1000+i`). **Do not reorder — the `6000–6999` file range, the `7000+i` Armor range, and the `loot_tile` id must be checked before the `1000+i` program range.** The old single `6000` `MEMORY_UNIT` `loot_tile` id is removed (`6000` is now the first file id).
...
3. Configure properties (`program_name`, `type`, `effect_type`, `memory_cost`, `strength`, `price`, and optionally `damage_dice` — `0` = flat `strength` per Black ICE hit; `>0` = roll `1D{damage_dice}` per hit, e.g. Sword = 6).
