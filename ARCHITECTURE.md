...
- **Properties**:
  ...
  - **Per-tile ICE config** (BLACK_ICE tiles; authored in the datafort designer's ICE editor):
    ...
    - There are no per-tile scalar stat overrides and no tier templates — `max_integrity` is derived 1:1 from `program.strength` at spawn, movement is STR-based (`program.strength` spaces/turn), and tracing behavior lives in program-specific logic for Watchdog-type programs (`WatchdogProgram.take_ice_turn` rolls the trace check and emits `trace_succeeded`; no scalar `traces` flag).
...
- **EffectType Enum**:
  ...
  - `REVEAL_NODES`: Detection utility that lifts fog of war without moving the runner. **Sensor** (base dispatch `_execute_sensor`) sweeps a square radius of `strength` tiles around the runner, setting `is_explored=true` (structure revealed even behind walls; `is_visible` left to the per-turn recalc, so sensed tiles show as "explored not visible"). **Probe** (`probe_program.gd` → `_execute_probe`) targets a single tile within line of sight, reveals it, and reports any ICE/netrunner stationed there. Both consume 1 action; neither consumes movement.
  - `MODIFY_MU`: Per-run utility boost (one active instance; transient — resets on scene change). **Toolbox** (base dispatch `_execute_toolbox`) raises the deck's effective MU ceiling (`netrunner.temp_memory_bonus = strength`) so the runner can copy/equip more; all in-run free-MU checks use `netrunner.effective_max_memory()`. **Speed** (`speed_program.gd` → `_execute_speed`) raises the initiative speed bonus (`netrunner.temp_speed_bonus = strength`), added to `nr_init` in both initiative sites. Both consume 1 action; re-activation is a no-op while the boost is running.
  ...
  - `DEMON`: A program shell carrying N other programs as **subroutines** (Imp=2, Afreet=3, Succubus=4, Balron=5). Rezzed as a single `DemonNode` (a `RezzedProgram` subclass); the runner commands it (1 action) to fire any loaded subroutine, which executes its original effect but uses the **Demon core's** `strength` (not the subroutine's own) — the faithful CP2020 multi-program-in-one tradeoff. See §4.12 for the full Demon model.
- **Properties**: `program_name`, `type`, `effect_type`, `memory_cost` (MU), `strength`, `price`, `icon` (Texture2D — workbench list rows only), `description` (one-line summary shown in the workbench detail card), `glyph` + `color` (on-map visual identity — see below), `glyph_offset` (optional `Vector2` nudge for glyphs that the auto-centring can't fix — see "Auto-centring" below; default `ZERO`), `damage_dice` (optional: `0` = use flat `strength` as damage on each Black ICE hit, the default for all existing programs; `>0` = roll `1D{damage_dice}` per hit instead, e.g. **Sword** sets `6` to roll 1D6 per hit), `damage_dice_count` (optional: number of dice to roll; default `1`; e.g. **Hellhound** sets `damage_dice=10, damage_dice_count=2` to roll 2D10), `mesh_scene` (optional `PackedScene` — per-program 3D model used when the program is rezzed onto the net; see §6.1 step 8). Backward-compatible: every existing `.tres` keeps flat-strength damage without re-saving.
...
### 4.11 `NetFile` ([cp2020_net_file.gd](scripts/resources/cp2020_net_file.gd)) — *NEW*
...
### 4.12 Demon Class ([demon_program.gd](scripts/programs/demon_program.gd) / [cp2020_demon.gd](scripts/resources/cp2020_demon.gd)) — *NEW*
A **Demon** is a CP2020 program shell that carries several other programs as **subroutines** (Imp=2, Afreet=3, Succubus=4, Balron=5). The runner loads subroutines into the Demon at the **workbench** (deck-prep), rezzes the Demon onto the net as a single node, then commands it to fire any loaded subroutine — each subroutine executes its original effect but uses the **Demon core's** `strength` (not the subroutine's own). This generalizes the `RezzedProgram` system from "one program per node" to "one shell carrying multiple subroutines per node".
- **`DemonProgram`** (`scripts/programs/demon_program.gd`, `class_name DemonProgram extends NetProgram`):
  - `max_subroutines: int` (2/3/4/5 by level) — gates slot count.
  - `demon_level: int` (1=Imp, 2=Afret, 3=Succubus, 4=Balron) — descriptive only.
  - `assigned_subroutines: Array[NetProgram]` — references to installed-program copies loaded at the workbench (empty by default).
  - `ALLOWED_SUB_EFFECTS` const — the executable effect types a Demon may carry: `DEREZ_ICE`, `DAMAGE_RUNNER`, `CRASH_CPU`, `SHIELD`, `ARMOR`. (Expandable later.) Demons cannot nest other Demons.
  - `execute_runner_action()` override: logs that Demons must be rezzed first (never fired from the deck).
- **`DemonNode`** (`scripts/resources/cp2020_demon.gd`, `class_name DemonNode extends RezzedProgram`): is-a `RezzedProgram`, so it inherits auto-follow, integrity-from-STR, `take_damage`, pathfinding, visibility, and Killer-ICE targeting. Adds:
  - `subroutines: Array[NetProgram]` — duplicated copies of the Demon's `assigned_subroutines`, each with `strength` **overridden to the Demon core's STR** (the faithful CP2020 "subroutines use the Demon's STR" rule). Populated by `setup_subroutines(demon)` (called by the game session at spawn, before `initialize()`).
  - `get_commandable_subroutines()` / `get_subroutine(index)` — the list the command menu iterates.
  - Visual: applies the Demon's own glyph/color (from `get_visual()`), not per-subroutine looks.
  - Lives in `rezzed_program_nodes` alongside plain rezzed attack programs, so `_tick_rezzed_programs` auto-follows it (inherited) and Killer ICE (which scans `ice.rezzed_programs`) targets it. **Demons are NOT worms** — they are visible and targetable by Killers; do not exclude them.
- **Rezzing** (`CP2020GameSession._rez_program`): `_is_rezzable()` returns true for `DemonProgram`; the DemonProgram branch spawns a `DemonNode`, duplicates the DemonProgram for the node's `.program` (stats/visual), sets `source_program` to the original installed prog (one-node-per-copy rule), calls `setup_subroutines(prog)` with the **original** installed prog (avoiding double-duplication of subroutines), and wires the standard rezzed signals.
- **Commanding** (`_command_demon(demon, sub_index, target_coord)`, action `"command_demon"`, 1 action): the interaction handler's Demon command menu (ids `8500+i`) emits `action_triggered("command_demon", target_coord, [DemonNode, subroutine_index])` — the payload rides the existing 3-arg signal as an `Array` (no new signal arity). The session unpacks it and dispatches on the **subroutine's** `effect_type` to the existing helpers (`_execute_ice_attack` / `execute_npc_attack` / `datafort.crash_cpu` / `_execute_shield` / `netrunner.raise_armor`), sourcing STR from the subroutine duplicate (already carrying the Demon's STR). Attack subroutines render a beam from the Demon to the target; SHIELD/ARMOR self-buff the runner (no beam).
- **Workbench assignment** (`cyberdeck_workbench.gd`): selecting a loaded DemonProgram enables the "⚙ SUBS" button → opens the "Configure Subroutines" window (built in code, modeled on the Purchase Unlocks window). The window lists N slot rows + a filtered candidate list (installed programs restricted to `ALLOWED_SUB_EFFECTS`, excluding Demons). Assign fills a chosen/next-free slot (references, not duplicates — subs are only read for effect_type + STR-override at spawn); Clear empties a slot; both persist via `RunState.save_run()`. **`_load_program_at` duplicates DemonPrograms on load** (with `assigned_subroutines` cleared) so workbench assignment never mutates the catalogue `.tres` or owned-program copy.
- **`.tres` files**: `data/imp.tres` (L1, 2 slots, STR 4, 4 MU, 1500 eb), `data/afreet.tres` (L2, 3 slots, STR 5, 6 MU, 3000 eb), `data/succubus.tres` (L3, 4 slots, STR 6, 8 MU, 5500 eb), `data/balron.tres` (L4, 5 slots, STR 8, 10 MU, 9000 eb). Subroutines empty by default (assigned at the workbench). Each has a distinct glyph/color. Like all programs, they appear in the Purchase Unlocks window (the data-catalogue scan loads any `data/*.tres` that `is NetProgram`) and the shop buy list once unlocked.

### 4.13 Deck Hardware Modules ([cp2020_deck_module.gd](scripts/resources/cp2020_deck_module.gd)) — *NEW*
A **DeckModule** is a piece of cyberdeck hardware the runner buys at the hub shop or loots from dataforts, then installs into a deck's generic upgrade slots at the workbench. Modules are swappable (free to install/uninstall) and **per-life** (lost on death with the deck). This is the CP2020 "deck construction" layer: instead of buying a whole new deck to raise one stat, the runner slots hardware modules that each boost a single stat. **NOT a NetProgram subclass** — modules are hardware, not software. They do not occupy deck MU and are never rezzed/fired. They are pure passive stat bonuses read via `Cyberdeck.effective_*()` accessors.
- **`DeckModule`** (`scripts/resources/cp2020_deck_module.gd`, `class_name DeckModule extends Resource`):
  - `ModuleEffect` enum: `MU` (+max MU), `SPEED` (+speed bonus → initiative), `DATA_WALL` (+Data Wall STR — display-only for now; `netrunner.max_health` is set in the scene, not from the deck), `INTERFACE` (+interface rank), `TRACE_REDUCTION` (adds the module's bonus to the Watchdog trace-check target, making the runner harder to trace — see Trace dampening below).
  - Fields: `module_name`, `effect_type`, `bonus_value: int`, `price: int`, `description`, `glyph`/`color` (for UI icon), `source_path` (for save/load, same pattern as `NetProgram`).
  - Static helpers: `effect_tag(effect)` → short tag ("MU"/"SPD"/"WALL"/"INT"/"TRACE"), `effect_label(effect)` → full label. `bonus_sign()` → "+N" for boosts, "−N" for trace reduction.
- **`Cyberdeck` accessors** (`scripts/resources/cp2020_cyberdecks.gd`): the deck gains `upgrade_slots: int` (default 3; starter deck=2, mid=3, high-end=4) and `installed_modules: Array[DeckModule]`. **Effective-stat accessors** sum base + installed-module bonuses: `effective_max_mu()`, `effective_speed_bonus()`, `effective_data_wall_strength()`, `effective_interface_rank()`, `effective_trace_reduction()`. All gameplay + UI stat reads MUST go through these accessors (never raw `.max_mu` etc.) so installed modules actually take effect. Slot management: `free_upgrade_slots()`, `can_install_module()`, `install_module(mod)`, `uninstall_module(mod)`. Slots are **generic** (any module type fits any slot; one module per slot).
- **Per-life inventory**: `RunState.owned_modules: Array[DeckModule]` (lost on death, like `owned_programs`). `buy_module()` / `add_module_loot()` / `install_module_to_deck()` / `uninstall_module_from_deck()` helpers. Saved/loaded via `RunStateData` — the per-life snapshot Resource in `scripts/autoload/run_state_data.gd`, preloaded by `RunState` (`owned_module_paths` + per-deck `installed_module_paths` in `owned_deck_entries`). Modules are `duplicate()`d at buy/loot/load time with `source_path` tagged so installed modules never mutate catalogue `.tres` files.
- **Persistent catalogue**: `MetaState.unlocked_modules: Array[String]` + `unlock_module()` / `has_module()`. Modules follow the same two-tier unlock model as decks and programs: the data-catalogue scan adds any `data/*.tres` that `is DeckModule` to the Purchase Unlocks window, where the runner pays the module's `price` to unlock the blueprint; only then does it appear in the shop BUY MODULES panel.
- **Workbench UI**: the "⬡ UPGRADES" button (next to "⚙ SUBS") opens the "Deck Upgrades" window (built in code, modeled on the Configure Subroutines window). Shows the deck's N slot rows (filled/empty) + an owned-modules library list + Install/Uninstall/Done buttons. Installing moves a module from `owned_modules` into `deck.installed_modules`; uninstalling moves it back. Both persist via `RunState.save_run()` and refresh the deck stat panel (which reads effective stats). The deck stat panel (`update_deck_ui`) and runner panel (`_refresh_netrunner_panel`) route all stat reads through `effective_*()` accessors.
- **Shop**: a "BUY MODULES" `ItemList` in the shop panel (parallel to `shop_buy_programs_list`), populated from `MetaState.unlocked_modules` minus owned. `buy_module()` deducts credits and duplicates the module into `owned_modules`.
- **Loot**: `CP2020TileData.loot_modules: Array[DeckModule]` (parallel to `loot_programs`). The `loot_tile` action (id `5000`, CONTROL_NODE right-click → "▼ Loot Node") picks up modules via `RunState.add_module_loot()` (which also calls `MetaState.unlock_module()` to add to the catalogue).
- **Trace dampening**: LDL hops add the **full** destination `trace_value` to `RunState.accumulated_trace` (building up trace makes Watchdog trace checks harder — the watchdog must roll `1D10+STR` ≥ the accumulated trace). A `TRACE_REDUCTION` options module (Trace Dampener) does **not** reduce this gain; instead its `bonus_value` is added to the Watchdog trace-check target (`accumulated_trace + effective_trace_reduction()` in `WatchdogProgram.take_ice_turn`), making the runner harder to pinpoint. No change to the trace reset lifecycle.
- **`.tres` files** (10 modules in `data/`): `mu_expansion_2` (+2 MU, 800 eb), `mu_expansion_5` (+5 MU, 2000 eb), `speed_booster_1` (+1 SPD, 1200 eb), `speed_booster_2` (+2 SPD, 3000 eb), `datawall_hardening_2` (+2 WALL, 1000 eb), `datawall_hardening_4` (+4 WALL, 2500 eb), `interface_coprocessor_1` (+1 INT, 1500 eb), `interface_coprocessor_2` (+2 INT, 3500 eb), `trace_dampener_3` (−3 TRACE, 1800 eb), `trace_dampener_5` (−5 TRACE, 4000 eb). Each has a distinct glyph/color.
...
- **Loot** (`_on_action_triggered "loot_tile"`): a free action (no turn consumed) for an adjacent, visible, un-looted `CONTROL_NODE` tile with program loot. Moves the tile's `loot_programs` into `RunState.loot` (each `duplicate()`d so cached `.tres` files aren't mutated), unlocks their resource paths in `MetaState`, adds `loot_credits` to `RunState.credits`, picks up `loot_modules` via `RunState.add_module_loot` (which also unlocks them in `MetaState`), and marks `tile.is_looted = true`. (The old single `MEMORY_UNIT` loot path has been replaced by per-file copy — `loot_tile` is now `CONTROL_NODE`-only.)
...
- **Watchdog Alarm & Beacons** (Detection/Alarm system — dual-role `DETECTION` effect):
  - **ICE-side alarm + trace** (`_on_ice_alarm_triggered()` / `_on_ice_trace_succeeded()`): connected to each `BlackIce.alarm_triggered` and `trace_succeeded` signal at spawn. When a DETECTION ICE (Watchdog) first sights the runner within its 20-space LoS, it emits `alarm_triggered`; the handler iterates all `ice_nodes` and calls `activate_alarm()` on every non-DETECTION ICE, waking dormant ICE to `PURSUE` (`_activated = true`, `current_state = PURSUE`). The Watchdog also rolls its CP2020 trace check (`1D10+STR` vs `RunState.accumulated_trace`); on success it emits `trace_succeeded`, and `_on_ice_trace_succeeded` rolls the meatspace security-dispatch countdown (see §5.x Security Dispatch). The Watchdog itself stays stationary — no pursuit, no attack. Initiative is handled naturally by the turn manager (the runner can kill it or retreat before it acts).
...
- **Turn Execution**: `_all_adversaries()` merges `datafort` + `ice_nodes` + `npc_nodes` into one array passed to `turn_manager.run_adversary_phase` (datafort first so it acts first). The turn manager calls `take_turn(target, layout)` on any node that has the method, so NPCs and the datafort drop in without changing its signature.
...
- **DETECTION ICE (Watchdog alarm + trace)**: a `BlackIce` with `effect_type = DETECTION` (value `10`) is a stationary alarm + tracing tripwire — it **does not pursue or attack**. In `take_turn`, the DETECTION case scans a 20-space LoS radius (`sight_range`) for the netrunner; on the first sighting `WatchdogProgram.take_ice_turn` emits `alarm_triggered` (once per ICE) and **rolls the CP2020 trace check**: `1D10 + program.strength` vs `RunState.accumulated_trace` (the runner's total LDL Trace Value — the defensive target number built from bouncing through LDLs on the world map). On a success it emits `trace_succeeded(program)`. A runner who didn't bounce through any LDLs (`accumulated_trace == 0`) is trivially pinpointed. The game session's `_on_ice_alarm_triggered()` then calls `activate_alarm()` on every other non-DETECTION ICE node, waking dormant ICE (`_activated = true`, `current_state = PURSUE`). See §5.1 for the alarm handler + netrunner-side beacon system, and the **Security Dispatch** section below for the trace-success consequence.
...
- **New signals**: `alarm_triggered` — emitted by a DETECTION ICE on its first sighting of the netrunner, connected to `game_session._on_ice_alarm_triggered()`. `trace_succeeded(program: NetProgram)` — emitted by a Watchdog when its trace check succeeds on first detection, connected to `game_session._on_ice_trace_succeeded(program)`.
- **Tracing ICE / Security Dispatch**: when a Watchdog's trace check succeeds (`1D10+STR ≥ accumulated_trace`), the runner's physical location is pinpointed. The game session's `_on_ice_trace_succeeded(program)` escalates the datafort's ICE (calls `_on_ice_alarm_triggered()` — no new NPCs are spawned) and rolls a **meatspace security-dispatch countdown** by the datafort's security tier: GREY/LEVEL_1 → `2D6` turns, LEVEL_2/LEVEL_3 → `1D10` turns, BLACK → `1D6` turns (lower tier = slower meatspace response). The countdown is stored in `RunState.security_dispatch_turns`, shown on the datafort HUD (`SecurityDispatchLabel`) and appended to the city grid / world map trace labels, and **ticks down each netrunner turn** in `_on_turn_ended`. At 0 → `_trigger_security_raid()` → **Busted permadeath** (meatspace team raids the runner's location while jacked in; `last_death_cause = "Traced"`). **Jacking out before 0 escapes the raid** (timer cleared on successful jack-out), but the existing `accumulated_trace >= BUSTED_THRESHOLD` bust check still applies on jack-out. Only the first successful trace rolls the timer; later traces are no-ops (the raid is already en route). The timer is **preserved** across in-datafort LDL travel and the datafort→City Grid return (mid-run), matching `accumulated_trace`; it is **cleared** on run-end (Datafort/City Grid/World Map Jack Out to Hub), return-to-world-map, flatline, and successful jack-out.
...
### 5.4b NPC Netrunner AI ([cp2020_npc_netrunner.gd](scripts/resources/cp2020_npc_netrunner.gd)) — *NEW*
...
### 5.4c Datafort CPU Adversary ([cp2020_datafort.gd](scripts/resources/cp2020_datafort.gd)) — *NEW*
...
- **Menu branches** (first match wins):
  ...
  - **Visible, adjacent, un-looted `CONTROL_NODE` with loot**: offer "▼ Loot Node" (the `loot_tile` action, now `CONTROL_NODE`-only — program loot moved off `MEMORY_UNIT`). Free action, no turn consumed. Its menu id is `5000` (the old `5000+i` CPU-crash range was removed in Phase 1, so `5000` is free). Kept **disjoint from the `6000–6999` file range** to avoid collision.
...
- `_on_menu_action_selected` checks ids in this order to avoid collision: LDL travel (`3000`/`3001`) → vertical travel (`3002`/`3003`) → NPC talk (`4000`) → **memory files (`6000+i` per file / `6999` Copy All)** → **Armor-raise (`7000+i`)** → **rez program (`8000+i`)** → **de-rez (`8100+i`)** → **rezzed anti-ICE attack (`8200+i`)** → **rezzed NPC attack (`8300+i`)** → **rezzed CPU crash (`8400+i`)** → **Demon command subroutine (`8500+i`)** → **CONTROL_NODE loot (`5000` / `loot_tile`)** → program use (`1000+i`). **Do not reorder — the `6000–6999` file range, the `7000+i` Armor range, the `8000–8599` rezzed-program/Demon ranges, and the `5000` `loot_tile` id must all be checked before the `1000+i` program range.** The old direct NPC-attack (`2000+i`) and CPU-crash (`5000+i`) ranges have been removed — attack programs must be rezzed first (Phase 1).
...
### 5.8 Cyberdeck Workbench / Hub UI ([cyberdeck_workbench.gd](scripts/ui/cyberdeck_workbench.gd))
...
- **LDL command panel**: a persistent `PanelContainer` docked to the right side of the HUD overlay (built in code, not in the `.tscn`). Lists every hackable LDL within Chebyshev ≤ 5 of the runner (each row: name / Sec / +Trace), plus an **ENTER \<city\> City Grid** button (visible only when the runner sits on a hub with a `city_grid_path`) and a **\> Jack Out to Hub** button. Replaces the old right-click popup (right-click no longer does anything).
  - **Hack**: CP2020 RAW LDL connection check — a **raw `1D10`** roll (no STAT + Skill added) must meet or exceed the destination's `security_code`. Success → teleport + add the **full** `trace_value` (the Trace Dampener module no longer reduces this; building up trace makes Watchdog trace checks harder). **Failure** → the line simply drops: no trace added, runner does **not** move, NetWatch is **not** automatically alerted (per RAW — NetWatch/raids only arise from in-datafort security programs or a sysop spotting you). The homebrew 1D6 caught-table / netcop-bust has been removed.
  - **Pay**: removed (paying gives away the runner's location, defeating the game's premise). `ldl_cost` on `CP2020WorldHub` is kept for save compatibility but no longer read at runtime.
...
- **Trace** (`accumulated_trace`): the total Trace Value of all LDLs passed through in the current Net run. It drives Watchdog trace checks (ICE must roll `1D10+STR ≥ trace` to pinpoint the runner) and is shown on the world map, city grid, and datafort HUDs. It is **reset to 0** on flatline, jack-out, and return-to-world-map; it is **preserved** across in-datafort LDL travel AND across the datafort→City Grid return (mid-run retreat keeps the trace). The **busted** check (`accumulated_trace >= BUSTED_THRESHOLD`) happens in `_on_jack_out_pressed` *before* trace is cleared.
- **Security Dispatch** (`security_dispatch_turns`): meatspace raid countdown set when a Watchdog trace check succeeds. Ticks down each netrunner turn; at 0 → Busted permadeath (`last_death_cause = "Traced"`). Rolled by tier: GREY/L1 = 2D6, L2/L3 = 1D10, BLACK = 1D6 turns. Jacking out before 0 escapes the raid (cleared); preserved across in-datafort LDL travel + datafort→City Grid return; cleared on run-end / flatline / successful jack-out.

### 5.12 Meta State — Persistent Catalogue ([meta_state.gd](scripts/autoload/meta_state.gd)) — *NEW*
...
### 5.14 Workbench Shop — Two-Tier Buy + Unlock ([cyberdeck_workbench.gd](scripts/ui/cyberdeck_workbench.gd)) — *NEW*
...
### 5.13 Game Over Scene ([game_over.gd](scripts/ui/game_over.gd) + `scenes/ui/GameOver.tscn`) — *NEW*
...
### 5.15 Combat Effect Animator ([cp2020_combat_effect_animator.gd](scripts/resources/cp2020_combat_effect_animator.gd)) — *NEW*
...
7. **For a Demon** (`effect_type = DEMON`): use `script = ExtResource("res://scripts/programs/demon_program.gd")` instead of the base `NetProgram` script, and set `demon_level` + `max_subroutines` (2/3/4/5). Leave `assigned_subroutines` empty — the runner fills slots at the workbench via the "⚙ SUBS" Configure Subroutines window (§4.12). Subroutines are restricted to `DemonProgram.ALLOWED_SUB_EFFECTS` (`DEREZ_ICE` / `DAMAGE_RUNNER` / `CRASH_CPU` / `SHIELD` / `ARMOR`); extending the allowed set also requires a dispatch branch in `CP2020GameSession._command_demon`. A Demon's own `strength` is the combat STR all its subroutines fire at.
8. **Optional per-program 3D model** (rezzed attack programs only — `DEREZ_ICE` / `DAMAGE_RUNNER` / `CRASH_CPU` / `DEMON`): set `mesh_scene` (a `PackedScene` `.tscn` authored from a `.glb`, with scale/rotation/position set in the editor) to give that program a unique 3D proxy when rezzed onto the net. Resolution is layered, mirroring the 2D `glyph`/`color` system: the program's own `mesh_scene` wins, then the matching Board3D per-effect-type slot (`rezzed_derez_mesh` / `rezzed_damage_mesh` / `rezzed_crash_mesh` / `rezzed_demon_mesh`), then the generic `rezzed_mesh` (the shared fallback). Leave `mesh_scene` null and the per-effect-type slot empty to inherit the generic model. Shield/Armor/Invisibility etc. buff the runner and never spawn a rezzed node, so `mesh_scene` is ignored for them (the runner glow shell covers Shield/Aegis). The model's own materials are kept (no material override when a scene is set).
...
5. If the tile hosts an entity (like BLACK_ICE/NETWATCH/NETRUNNER), add a `paint_tile` case, a spawn function + tier templates in [cp2020_game_session.gd](scripts/resources/cp2020_game_session.gd), and an editor side panel (mirror `build_ice_panel`/`build_npc_panel`).
...
- **Datafort does not control ICE**: The [CP2020Datafort](scripts/resources/cp2020_datafort.gd) adversary runs its **own** `resident_programs` against the runner; it does not command the Black ICE or NPC netrunners, which stay independent turn-manager adversaries. The datafort is prepended to `_all_adversaries()` so it acts first.
...
- **Helpers**: `type_tag(t)` / `type_label(t)` (static), `objective_text()` (instance — type-specific objective line).

### 7.2 State (per-life, in `RunState` / `RunStateData`)
- **Fields**: `active_mission: CP2020Mission` (null = none), `available_missions: Array[CP2020Mission]` (the rotating board), `mission_objective_met: bool`, `last_mission_refresh_time: float`. Constants `MISSION_REFRESH_SECONDS = 3600.0` (1 hour), `MISSION_BOARD_SIZE = 4`.
- **Seeding**: `start_new_life()` → `_seed_mission_board()` picks a random `MISSION_BOARD_SIZE` subset of the library. Older saves that predate the system load with an empty board and are auto-seeded in `_load_run`.
- **Refresh**: `check_mission_refresh()` (called by the workbench on entry + on tab switch to Missions) rotates the oldest board entry for a fresh library mission when ≥ `MISSION_REFRESH_SECONDS` of net-time has elapsed. The clock (`net_time_seconds`) only ticks while jacked in (advanced by each scene's `action_consumed` handler), so **the board does not advance at the hub**.
- **Accept/abandon/hand-in**: `accept_mission(m)` (one active max, removes from board), `abandon_mission()` (returns to board if objective unmet), `hand_in_mission()` (pays `reward_credits` into `credits`; for DATA_HARVEST also removes one copy of the target file from `carried_files` — the file is "delivered" to the fixer).
- **Objective notify hooks** (called by the game session): `notify_file_copied(file)` (DATA_HARVEST), `notify_action_at_coord(current_subnet_path, coord)` (SABOTAGE), `notify_position(current_subnet_path, coord)` (RECON). Each no-ops unless the active mission's type + subnet + coord/file match.
- **Persistence**: `RunStateData` stores `available_mission_paths` + `active_mission_path` + `mission_objective_met` + `last_mission_refresh_time`; `_load_run` reloads + duplicates from paths. Active mission + objective flag are **preserved across a successful jack-out** (so the runner can hand in) and **discarded on death/busted** (`reset()` clears them).

### 7.3 Gameplay hooks (`cp2020_game_session.gd`)
- **`current_subnet_path`** (new var): set in `load_subnet` (including mid-run LDL travel). Objective subnet checks compare against this, **not** `RunState.selected_subnet_path` (which is set once by the city-grid DIVE and is stale after LDL travel).
- **DATA_HARVEST**: `notify_file_copied` fires on `copy_file` and `copy_all_files` when the copied file's `file_name` matches the active mission's `target_file_name`. Hand-in re-verifies the file is still carried (`can_hand_in_mission` → `_carries_target_file`).
- **SABOTAGE**: `notify_action_at_coord` fires at the top of `_on_action_triggered` for `attack_with_rezzed` / `command_demon` / `loot_tile` when the action's `target_coord` matches the mission coord in the target subnet (covers CPU crash, ICE/NPC attack, node loot).
- **RECON**: a new `netrunner.position_changed` handler (`_on_netrunner_position_changed_mission`) sets the flag when the runner steps onto the target coord in the target subnet.
...
### 7.4 Workbench UI — Missions tab (`CyberdeckWorkbench.tscn`, tab index 2)
Added as a third tab in the `Tabs` `TabContainer` (Loadout=0, SHOP=1, MISSIONS=2). Scene-tree nodes referenced via `unique_name_in_owner` (`%MissionList`, `%MissionRefreshLabel`, `%AcceptMissionButton`, `%MissionTitle`/`%MissionType`/`%MissionLocation`/`%MissionObjective`/`%MissionReward`/`%MissionDesc`, `%ActiveMissionLabel`/`%ActiveObjectiveLabel`, `%HandInMissionButton`/`%AbandonMissionButton`).
- **Available-contracts list** (colour-coded: harvest=green, sabotage=red, recon=cyan) + **detail card** + **Accept Contract** button (disabled while a mission is active).
- **Active-contract status box** with live objective + completion indicator (`✓ READY TO HAND IN` when `can_hand_in_mission()`) and **Hand In** / **Abandon** buttons.
