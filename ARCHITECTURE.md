...
- **Menu branches** (first match wins):
  ...
  - **Runner's own tile** (visible): offer `SHIELD` defense programs (ids `1000+i`) and `ARMOR` defense programs (ids `7000+i`). Armor is a persistent passive absorber; raising it does not consume a turn action.
