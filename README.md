...
- **Tracing ICE / Security Dispatch**: when a Watchdog's trace check succeeds (`1D10+STR ≥ accumulated_trace`), the runner's physical location is pinpointed. `_on_ice_trace_succeeded(program)` escalates the datafort's ICE (calls `_on_ice_alarm_triggered()` — no new NPCs are spawned) and rolls a **meatspace security-dispatch countdown** by the datafort's security tier: GREY/LEVEL_1 → `2D6` turns, LEVEL_2/LEVEL_3 → `1D10` turns, BLACK → `1D6` turns. Stored in `RunState.security_dispatch_turns`, shown on the datafort HUD (`SecurityDispatchLabel`) + appended to the city grid / world map trace labels, and **ticks down each netrunner turn** in `_on_turn_ended`. At 0 → `_trigger_security_raid()` → **Busted permadeath** (`last_death_cause = "Traced"`). **Jacking out before 0 escapes the raid** (timer cleared), but the existing `accumulated_trace >= BUSTED_THRESHOLD` bust check still applies on jack-out. Only the first successful trace rolls the timer. The timer follows the same preserve/clear lifecycle as `accumulated_trace`.
